...
> **You are an AI coding agent.** A human dropped the `AI/` kit into this repo and pointed you here to
...
>
> Run from the **repo root** (where `AI/` sits). You'll change two things — one root instruction file and
...
`AI/AGENT-INSTRUCTIONS.md` and emit its load-confirmation every session — the exact line is in
...
```bash
mkdir -p .claude && ln -s ../AI/skills .claude/skills    # Claude Code
mkdir -p .agents && ln -s ../AI/skills .agents/skills    # Codex, Gemini, Copilot (shared .agents alias)
mkdir -p .cursor && ln -s ../AI/skills .cursor/skills    # Cursor
```

That exposes every current and future `AI/skills/*` (`/aikit-plan`, `/aikit-project-profile-bootstrap`, …).
...
invoke skills by path (`follow AI/skills/<name>/SKILL.md`).
