...
| Skill | When to use |
| --- | --- |
| [aikit-plan](aikit-plan/SKILL.md) | Stand up / maintain `ai-progress/` for non-trivial work — one uniform effort folder per goal (ROADMAP + LOG + gate-authored phase files + close-out SUMMARY), for tasks and epics alike. Covers creating an effort, phase gates, cold resume, blockers, re-scoping, out-of-scope findings, closing as done or cancelled, and legacy efforts (never auto-migrated). Walks `AGENT-INSTRUCTIONS.md` §4; the file format lives in its [progress contract](aikit-plan/references/progress-contract.md) and the findings ladder in its [findings workflow](aikit-plan/references/findings-workflow.md). |
...
| [aikit-switch-visibility](aikit-switch-visibility/SKILL.md) | Flip the kit between `shared` and `local-only` after adoption — untrack/re-track the kit, `ai-progress/`, the kit-only pointer, and the skills symlinks together; update the Config-visibility Mode; surface the team-impact warnings (§5.2). |
